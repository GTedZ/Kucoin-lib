...
let api = function everything(APIKEY = false, APISecret = false, options = { hedgeMode: false, fetchFloats: false, useServerTime: false, recvWindow: 5000, offset: 0 }) {
    ...
    if (options.offset) this.timestampOffset_offset = options.offset; else this.timestampOffset_offset = 0;
    ...
    this.ping = async (reconnect = false, tries = -1) => {
        ...
        this.latency = resp.roundtrip_time_millis;
        return resp;
    }
    ...
    this.wallet = {
        ...
        depositHistory: (coin = undefined, status = undefined, startTime = undefined, endTime = undefined, offset = undefined, limit = undefined, opts = {}) => {
            const params = {
                baseURL: sapi,
        ...
        tradeFee: (symbol = undefined, opts = {}) => {
            const params = {
                baseURL: sapi,
        ...
        transferHistory: async (type, startTime = undefined, endTime = undefined, current = undefined, size = undefined, fromSymbol = undefined, toSymbol = undefined, opts = {}) => {
            ...
            if (fromSymbol) options.fromSymbol = fromSymbol;
            if (toSymbol) options.toSymbol = toSymbol;
    ...
    this.websockets = {
         ...
         * serverTime: Promise < { result: { serverTime: UNIX_timestamp }, rateLimits: Array } | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } > 
         * exchangeInfo(symbol_or_symbols:undefined|String|Array<String>, permissions:undefined|String|Array<String>, options: {mapped: boolean|undefined}): Promise < { result, rateLimits: Array } | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } >
         * account(mappedBalance: Boolean|undefined, activeAssetsOnly: boolean|undefined, opts: {recvWindow: Integer} |undefined): Promise < { result, rateLimits: Array } | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } > 
         * orderBook(symbol:String, limit:number|undefined): Promise < { result: { lastUpdateId:Number, bids: Array<Array>, asks: Array<Array> }, rateLimits:Array | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } >
         * recentTrades(symbol:String, limit:number|undefined): Promise < { result: Array, rateLimits: Array } | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } >
         * trades(symbol:String, limit:number|undefined, fromId: number|undefined): Promise < { result: Array, rateLimits: Array } | { error: { status:Integer, code: number, msg:String, rateLimits: Array } } >
         * aggTrades(symbol:String, limit:number|undefined, fromId: number|undefined, startTime: UNIX_timestamp|null, endTime: UNIX_timestamp|undefined): Promise < { result: Array<Array>, rateLimits: Array } | { error: { status:Integer, code:number, msg:String, rateLimits: Array } }>
         * candlesticks(symbol:String, interval:number|undefined, limit:number|undefined, startTime:UNIX_timestamp, endTime: UNIX_timestamp): Promise < { result: Array<Array>, rateLimits: Array } | { error: { status:Integer, code:number, msg:String, rateLimits: Array } } >
         * > } object }
         */
        websocketAPI: async function () {
            ...
            WS_Object.sendPrivateMessage = async (OBJ, signed = false, apiKeyOnly = false) => {
                ...
                if (signed) {
                    if (!newOBJ.params) newOBJ.params = {};
                    newOBJ.params.apiKey = binance.APIKEY;
                    newOBJ.params.timestamp = Date.now() + binance.timestamp_offset;
                    const signature = createSignature(OBJ.params);
                    newOBJ.params.signature = signature;
                } else if (apiKeyOnly) {
                    if (!newOBJ.params) newOBJ.params = {};
                    newOBJ.params.apiKey = binance.APIKEY;
                }
            ...
            WS_Object.privateMessage = (msg) => {
                ...
                if (msg.error) {
                    const rateLimits = msg.rateLimits;
                    for (let key of Object.keys(msg.error)) {
                        msg[key] = msg.error[key];
                    }
                    delete msg.rateLimits;
                    delete msg.error;
                    msg.rateLimits = rateLimits;
                } else {
                    delete msg.status;
                }
    ...
    const request = async (params, options = {}, type = 'default') => {
        ...
        for (let key of Object.keys(options)) {
            if (options[key] == undefined) delete params[key];
        }
    ...
    function PREP_WS_OBJECT(WS_Object) {
        ...
        WS_Object.account = async (mappedBalance = false, activeAssetsOnly = false, opts = {}) => {
            ...
            const resp = await WS_Object.sendPrivateMessage(obj, true);
            if (resp.error) return resp;
        ...
        WS_Object.orderBook = (symbol, limit = undefined, depth = undefined) => {
            const obj = {
                method: `depth`,
                params: {
                    symbol: symbol,
        ...
        WS_Object.recentTrades = (symbol, limit = undefined) => {
            const obj = {
        ...
        WS_Object.trades = (symbol, limit = undefined, fromId = undefined) => {
            const obj = {
            ...
            return WS_Object.sendPrivateMessage(obj, false, true);
        }

        WS_Object.aggTrades = (symbol, limit = undefined, fromId = undefined, startTime = undefined, endTime = undefined) => {
            const obj = {
        ...
        WS_Object.candlesticks = (symbol, interval, limit = undefined, startTime = undefined, endTime = undefined) => {
            const obj = {
                method: 'klines',
                params: {
                    symbol,
